...
public class CalypsoCertificateDemoWithFiles {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      printCertificateCompact(caCertificate, 4);
      ...
      printCertificateCompact(cardCertificate, 4);
  ...
  /** Displays a certificate in compact format (4 lines max). */
  private static void printCertificateCompact(byte[] certificate, int maxLines) {
    ...
    if (hex.length() > lineLength * maxLines) {
      System.out.println("   ... (" + (certificate.length - (maxLines * 32)) + " bytes remaining)");
    }
  }
}
